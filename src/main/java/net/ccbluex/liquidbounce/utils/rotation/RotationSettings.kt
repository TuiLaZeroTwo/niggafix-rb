...
open class RotationSettings(owner: Module, generalApply: () -> Boolean = { true }) : Configurable("RotationSettings") {
    ...
    open val strafeModeValue = choices("StrafeMode", arrayOf("Off", "Strict", "Silent"), "Silent") { rotationsActive && applyServerSide && generalApply() }
    ...
    fun applyStrafeFix() {
        ...
        val rinStrafe = LiquidBounce.moduleManager.getModule(RinStrafe::class.java) as? RinStrafe
        if (rinStrafe?.state == true) {
